...
int m,n,B,cur_B;
void doublecheck(int x,int y,int start_x,int start_y,bool**visited,int**dist){
    int nx,ny;
    nx = x-1; ny = y;
    if((dist[nx-1][ny]==-1)&&(dist[nx][ny-1]==-1)&&(dist[nx][ny+1]==-1))
        dist[nx][ny]=-1;
    nx = x+1; ny = y;
    if((dist[nx+1][ny]==-1)&&(dist[nx][ny-1]==-1)&&(dist[nx][ny+1]==-1))
        dist[nx][ny]=-1;
    nx = x; ny = y-1;
    if((dist[nx-1][ny]==-1)&&(dist[nx+1][ny]==-1)&&(dist[nx][ny-1]==-1))
        dist[nx][ny]=-1;
        nx = x; ny = y+1;
    if((dist[nx-1][ny]==-1)&&(dist[nx+1][ny]==-1)&&(dist[nx][ny+1]==-1))
        dist[nx][ny]=-1;
...
void check(int x,int y,int indir,int outdir,int start_x,int start_y,bool**visited,int**dist,int**dist2){
    ...
    &&(dist[x-DIREC[outdir][0]][y-DIREC[outdir][1]]==-1))
     {dist[x][y] = -1;
     //doublecheck(x,y,start_x,start_y,visited,dist);
     }
     return;
}
...
void walk(int cur_x,int cur_y,int dir,int start_x,int start_y,bool**visited,int**dist,int**dist2){  //in_dir
        ...
        ||(cur_B==dist2[cur_x][cur_y])
                ...
                cout<<"H";
                if((dist[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]]==-1)&&
                    (dist[cur_x+DIREC[(dir+2)%4][0]][cur_y+DIREC[(dir+2)%4][1]]==-1)&&
                    (dist[cur_x+DIREC[(dir+3)%4][0]][cur_y+DIREC[(dir+3)%4][1]]==-1))
                    dist[cur_x][cur_y] = -1;
                gohome(cur_x,cur_y,(dir+2)%4,start_x,start_y,visited,dist,dist2);
   ...
   for(int i=0;i<=m+1;i++) {
    ...
    if(cur_x==start_x&&cur_y==start_y) {
        ...
        if(count!=0&&dist[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]]==dist2[cur_x][cur_y]+1
                &&(!visited[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]])
                    &&cur_B-1>=dist2[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]])
                    {
                        check(cur_x,cur_y,dir,(dir+3)%4,start_x,start_y,visited,dist,dist2);
        ...
        else if(count!=0&&dist[cur_x+DIREC[(dir+2)%4][0]][cur_y+DIREC[(dir+2)%4][1]]==dist2[cur_x][cur_y]+1
        ...
        else if(count!=0&&dist[cur_x+DIREC[(dir+3)%4][0]][cur_y+DIREC[(dir+3)%4][1]]==dist2[cur_x][cur_y]+1
            ...
            if(count!=0&&dist[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]]!=-1
                ...
                if(count!=0&&dist2[cur_x+DIREC[(dir+1)%4][0]][cur_y+DIREC[(dir+1)%4][1]]!=-1
                   ...
                   else gohome(cur_x,cur_y,(dir+2)%4,start_x,start_y,visited,dist,dist2);
                }
            }
        }
    }
