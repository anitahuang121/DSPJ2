...
void doublecheck(int x,int y,int start_x,int start_y,bool**visited,int**dist){
    int nx,ny;
    nx = x-1; ny = y;
    if((dist[nx-1][ny]==-1)&&(dist[nx+1][ny]==-1)&&(dist[nx][ny-1]==-1)&&(dist[nx][ny+1]==-1))
        dist[nx][ny]=-1;
    nx = x+1; ny = y;
    if((dist[nx-1][ny]==-1)&&(dist[nx+1][ny]==-1)&&(dist[nx][ny-1]==-1)&&(dist[nx][ny+1]==-1))
        dist[nx][ny]=-1;
...
void gohome(int cur_x,int cur_y,int dir,int start_x,int start_y,bool**visited,int**dist,int**dist2){  //indir
    ...
    cout<<"home"<<cur_x<<","<<cur_y<<",d:"<<dist[cur_x][cur_y]<<",dir:"<<dir<<",count:"<<count<<"B:"<<cur_B<<endl;
    if(cur_x==start_x&&cur_y==start_y) {cur_B = B; walk(cur_x,cur_y,(dir+2)%4,start_x,start_y,visited,dist,dist2); return;}
    else{
        if(count==0) return;
